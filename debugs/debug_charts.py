...
print(
    f"MONTO - Min: {df['MONTO'].min()}, Max: {df['MONTO'].max()}, Mean: {df['MONTO'].mean()}"
)
...
if "MES_NUM" in df_processed.columns:
    print(
        f"MES_NUM - Min: {df_processed['MES_NUM'].min()}, Max: {df_processed['MES_NUM'].max()}"
    )
...
if amount_col:
    print(
        f"Valores en {amount_col} - Min: {df_processed[amount_col].min()}, Max: {df_processed[amount_col].max()}"
    )
...
if amount_col and "AÑO" in df_processed.columns and "MES_NUM" in df_processed.columns:
    ...
    if monthly is not None:
        ...
        if "monto_total" in historico.columns:
            print(
                f"monto_total - Min: {historico['monto_total'].min()}, Max: {historico['monto_total'].max()}"
            )
