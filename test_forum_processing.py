...
def test_process_single_file():
    ...
    processed_castigo = process_single_file(castigo_df, "castigo_ejemplo.xlsx", "Castigo")
    ...
    print(processed_castigo[['ORIGEN', 'CONTRATO', 'RUT', 'NOMBRE CLIENTE', 'MONTO CASIIGO', 'FECHA CASTIGO', 'CARTERA']])
    print()
    
    # Check that RUT was cleaned (dash removed)
    assert all('-' not in rut for rut in processed_castigo['RUT']), "RUT should not contain dashes"
    assert processed_castigo['RUT'].iloc[0] == '19513991', f"Expected '19513991', got '{processed_castigo['RUT'].iloc[0]}'"
    
    # Check that ORIGEN is set correctly
    assert all(origen == 'castigo_ejemplo' for origen in processed_castigo['ORIGEN']), "ORIGEN should be filename without extension"
    
    # Test with vigente data
    vigente_df = create_sample_vigente_data()
    processed_vigente = process_single_file(vigente_df, "vigente_ejemplo.xlsx", "Vigente")
    
    print("Processed vigente data:")
    print(processed_vigente[['ORIGEN', 'CONTRATO', 'RUT', 'NOMBRE CLIENTE', 'MONTO CASIIGO', 'FECHA CASTIGO', 'CARTERA']])
    print()
    
    # Check that RUT was cleaned
    assert all('-' not in rut for rut in processed_vigente['RUT']), "RUT should not contain dashes"
    assert processed_vigente['ORIGEN'].iloc[0] == 'vigente_ejemplo', "ORIGEN should be filename without extension"
...
def test_process_forum_data():
    ...
    combined_df = process_forum_data(castigo_df, vigente_df, "castigo_data.xlsx", "vigente_data.xlsx")
    ...
    assert all('-' not in rut for rut in combined_df['RUT']), "RUT column should not contain dashes"
    
    # Check that empty columns are present and empty
    assert all(etapa == '' for etapa in combined_df['ETAPA DEMANDA']), "ETAPA DEMANDA should be empty"
    assert all(ciudad == '' for ciudad in combined_df['CIUDAD']), "CIUDAD should be empty"
    assert all(tipo_gestion == '' for tipo_gestion in combined_df['Tipo gestión']), "Tipo gestión should be empty"
    
    # Check column order
    expected_order = [
        'ORIGEN', 'CONTRATO', 'RUT', 'NOMBRE CLIENTE', 'MONTO CASIIGO',
        'ETAPA DEMANDA', 'FECHA CASTIGO', 'CIUDAD', 'CARTERA', 'Tipo gestión'
    ]
